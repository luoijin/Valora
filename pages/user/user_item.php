            ...
            <div class="item-detail">
              ...
              <div class="item-info">
                ...
                <div class="item-action-buttons">
                  ...
                  <button class="item-btn item-btn-icon" id="wishlistBtn" title="Add to wishlist">
...
<script>
    ...
    if (wishlistBtn) {
        ...
        const productId = wishlistBtn.getAttribute('data-product-id') || '<?php echo $product['id']; ?>';
        const productName = "<?php echo htmlspecialchars($product['name'], ENT_QUOTES); ?>";
        const productImage = "<?php echo isset($product['image_path']) ? '../../' . preg_replace('/^\\.+\\//', '', $product['image_path']) : ''; ?>";

        const loadWishlist = () => {
            try { return JSON.parse(localStorage.getItem(storageKey)) || []; } catch(e){ return []; }
        };
        const saveWishlist = (list) => localStorage.setItem(storageKey, JSON.stringify(list));
        const isInWishlist = () => loadWishlist().some(item => item.id == productId);
        const setHeart = (active) => {
        ...
        wishlistBtn.addEventListener('click', () => {
            ...
            if (isInWishlist()) {
                ...
                list.push({ id: productId, name: productName, image: productImage });
                saveWishlist(list);
                setHeart(true);
